...
# Получаем строки с представлением объектов класса function,
# которые были созданы при определении наших функций
...
# ошибка связана с тем, что в функции num_1 уже объявлен обязательным, позиционным
# аргументом, и одновременно мы пытаемся передать еще одно значение, но уже с
# прямым указанием на наименование переменной внутри функции. Интерпретатор
# видит два значения для одного и того же аргумента и вызывает исключение
...
# выполняется, функция возвращает полученный из sum результат
...
# его пар ключ-значение и передача их, как именованных аргументов:
